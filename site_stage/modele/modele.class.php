<?php
    class Modele {
            ...
            if ($nb_articleRes == 0){echo "";}
                ...
                for ($i=0;$i<$nb_articleRes;$i++)
                {
                    echo 
                        "<li>
                            <div class='cart-item'>
                                <img src='".$info_article[$i]["img"]."' class='image-container'>
            ...
            if ($nb_standRes == 0){echo "";}
                ...
                for ($i=0;$i<$nb_standRes;$i++)
                {
                    echo 
                        "<li>
                            <div class='cart-item'>
                                <img src='".$info_stand[$i]["img"]."' class='image-container'>
                                <div class='item-details'>
        ...
        public function getArticle_Panier_qte($id_reservation){
            $requete = "select id_article,quantite from reserverarticle;"; 
            $exec = $this->unPdo->prepare ($requete);
            $exec->execute ();
            $res = $exec->fetchAll();
        ...
        public function remplirSommairePanierArticle(){
            ...
            if ($nb_standRes == 0){echo "";}
                ...
                { 
                    echo 
                    "<li>
                        <p>Article : ".$info_article[$i]["nom_article"]." ".$i." x ".$info_article_qte[$info_article[$i]["id_article"]]." = ".$info_article[$i]["prix"]*$info_article_qte[$info_article[$i]["id_article"]]."€</p>
                    </li>";
                }
